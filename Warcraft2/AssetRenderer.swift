class AssetRenderer {
    ...
    init(colors: GraphicRecolorMap, tilesets: [GraphicMulticolorTileset], markerTileset: GraphicTileset, corpseTileset: GraphicTileset, fireTileset: [GraphicTileset], buildingDeath: GraphicTileset, arrowTileset: GraphicTileset, player: PlayerData, map: AssetDecoratedMap) {
        ...
        pixelColors[PlayerColor.none.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "none"), cindex: 0)
        pixelColors[PlayerColor.blue.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "blue"), cindex: 0)
        pixelColors[PlayerColor.red.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "red"), cindex: 0)
        pixelColors[PlayerColor.green.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "green"), cindex: 0)
        pixelColors[PlayerColor.purple.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "purple"), cindex: 0)
        pixelColors[PlayerColor.orange.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "orange"), cindex: 0)
        pixelColors[PlayerColor.yellow.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "yellow"), cindex: 0)
        pixelColors[PlayerColor.black.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "black"), cindex: 0)
        pixelColors[PlayerColor.white.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "white"), cindex: 0)
        pixelColors[PlayerColor.max.rawValue] = colors.colorValue(gindex: colors.findColor(colorname: "self"), cindex: 0)
        pixelColors[PlayerColor.max.rawValue + 1] = colors.colorValue(gindex: colors.findColor(colorname: "enemy"), cindex: 0)
        pixelColors[PlayerColor.max.rawValue + 2] = colors.colorValue(gindex: colors.findColor(colorname: "building"), cindex: 0)
        ...
        for directionName in ["decay-n", "decay-ne", "decay-e", "decay-se", "decay-s", "decay-sw", "decay-w", "decay-nw"] {
            ...
            while true {
                tileIndex = corpseTileset.findTile(with: directionName + String(stepIndex))
                if tileIndex >= 0 {
        ...
        for directionName in ["attack-n", "attack-ne", "attack-e", "attack-se", "attack-s", "attack-sw", "attack-w", "attack-nw"] {
            ...
            while true {
                tileIndex = arrowTileset.findTile(with: directionName + String(stepIndex))
                if tileIndex >= 0 {
        ...
        for tileset in tilesets {
            printDebug("Checking Walk on" + String(typeIndex), level: .low)
            for directionName in ["walk-n", "walk-ne", "walk-e", "walk-se", "walk-s", "walk-sw", "walk-w", "walk-nw"] {
                var stepIndex: Int = 0
                var tileIndex: Int
                while true {
                    tileIndex = tileset.findTile(with: directionName + String(stepIndex))
                    if tileIndex >= 0 {
            ...
            printDebug("Checking Construct on" + String(typeIndex), level: .low)
            var stepIndex: Int = 0
            var tileIndex: Int
            while true {
                tileIndex = tileset.findTile(with: "construct-" + String(stepIndex))
                if tileIndex >= 0 {
            ...
            printDebug("Checking Gold on" + String(typeIndex), level: .low)
            ...
            printDebug("Checking Lumber on" + String(typeIndex), level: .low)
            ...
            printDebug("Checking Attack on" + String(typeIndex), level: .low)
            for directionName in ["attack-n","attack-ne","attack-e","attack-se","attack-s","attack-sw","attack-w","attack-nw"] {
                var stepIndex: Int = 0
                var tileIndex: Int
                while true {
            ...
            if attackIndices[typeIndex].count == 0 {
                var tileIndex: Int
                for _ in 0..<Direction.max.rawValue {
            ...
            printDebug("Checking Death on" + String(typeIndex), level: .low)
            ...
            printDebug("Checking None on" + String(typeIndex), level: .low)
            ...
            printDebug("Checking Build on" + String(typeIndex), level: .low)
            ...
            printDebug("Checking Place on" + String(typeIndex), level: .low)
            placeIndices[typeIndex].append(tileset.findTile(with: "place"))
            printDebug("Done checking type" + String(typeIndex), level: .low)
            typeIndex += 1
        }
    }
